...
EFB Compiler extends brainfuck by using extra symbols. The output will be valid brainfuck code that can run on any interpreter/compiler. If you are interested in the process of making this compiler, please go to the [Introduction](docs/Introduction.md). To find out more about the the syntax have a look at [EBF language introduction](docs/EBFLang.md)

## Serious use: Zozotez - a LISP interpreter implemented in EBF ##
The high level goal was to be able to implement a LISP interpreter which runs on brainfuck and bootstrapped with EBF.  I have spawned a new project called **[Zozotez for creating LISP with EBF](http://sylwester.no/zozotez/)**.
